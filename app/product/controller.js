...
const update = async (req, res, next) => {
  try {
    ...
    if (req.file) {
      ...
      src.on('end', async () => {
        try {
          let dataProduct = await Product.findById(id);
          let currImg = `${config.rootPath}/public/images/products/${dataProduct.image_url}`;
...
const index = async (req, res, next) => {
  try {
    ...
    if (category.length) {
      let categoryTemp = await Category.findOne({
        name: { $regex: `${category}`, $options: 'i' },
      });

      if (categoryTemp) {
        criteria = { ...criteria, category: categoryTemp._id };
      }
    }

    if (tags.length) {
      let tagsTemp = await Tag.find({ name: { $in: tags } });
      if (tagsTemp.length > 0) {
        criteria = {
          ...criteria,
          tags: { $in: tagsTemp.map((tag) => tag._id) },
        };
      }
    }
